# pyopenxr version is based on openxr version...
# except the patch number is 100 * openxr patch number + pyopenxr patch number

XR_VERSION_MAJOR = 1
XR_VERSION_MINOR = 0
XR_VERSION_PATCH = 17
XR_CURRENT_API_VERSION = "1.0.17"

PYOPENXR_VERSION_MAJOR = 1
PYOPENXR_VERSION_MINOR = 0
PYOPENXR_VERSION_PATCH = 1701
PYOPENXR_VERSION_PATCH_INCREMENTAL = 1
PYOPENXR_VERSION = "1.0.1701"

__version__ = PYOPENXR_VERSION

__all__ = [
    "XR_VERSION_MAJOR",
    "XR_VERSION_MINOR",
    "XR_VERSION_PATCH",
    "XR_CURRENT_API_VERSION",
    "PYOPENXR_VERSION_MAJOR",
    "PYOPENXR_VERSION_MINOR",
    "PYOPENXR_VERSION_PATCH",
    "PYOPENXR_VERSION_PATCH_INCREMENTAL",
    "PYOPENXR_VERSION",
]
